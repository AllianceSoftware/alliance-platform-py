...
def _generated_launcher_source(contents: str) -> str | None:
    if 'export ALLIANCE_DEV_INVOCATION_NAME="bin/dev"' not in contents:
        return None
    for line in contents.splitlines():
        if not line.startswith("default_tool_source="):
            continue
        try:
        ...
        if len(assignment) != 1:
            return None
        name, separator, value = assignment[0].partition("=")
        if name == "default_tool_source" and separator:
            return value
    return None
    ...
    *,
    ...
    if launcher.exists():
        if not launcher.is_file():
            raise DevError(f"Cannot update {launcher}; it is not a file")
        previous_contents = launcher.read_text()
        previous_source = _generated_launcher_source(previous_contents)
        if previous_source is None and not force:
            raise DevError(f"Refusing to overwrite unrecognized {launcher}; review it or pass --force")
    ...
    output.print(f"[bold green]Alliance development launcher {state}.[/bold green]")
    output.print(f"  Path:   {launcher}")
    if previous_source is not None:
        output.print(f"  Before: {previous_source}")
    output.print(f"  Source: {source}")
    return LauncherUpdateResult(
        repo=repo,
        launcher=launcher,
        state=state,
