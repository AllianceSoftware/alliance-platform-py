...
class BundlerAssetContextMiddleware:
    """Middleware that wraps requests in a :class:`~alliance_platform_frontend.bundler.context.BundlerAssetContext` manager

    Within a Django request you can call :meth:`~alliance_platform_frontend.bundler.context.BundlerAssetContext.get_current` and
    it will return context instance for the current request. :class:`~alliance_platform_frontend.bundler.context.BundlerAssetContext`
    does this on init.
    ...
    - :class:`~alliance_platform_frontend.bundler.context.SSRItem` can be queued for server side rendering using
      :meth:`~alliance_platform_frontend.bundler.context.BundlerAssetContext.queue_ssr`. The caller (e.g. a template node) must
