...
def get_all_templates_files():
    ...
    for dir in dirs:
        ...
        for excl in settings.FRONTEND_EXTRACT_ASSETS_EXCLUDE_DIRS:
...
def extract_asset_filenames():
    ...
    with BundlerAssetContext() as asset_context:
        ...
        for file in get_all_templates_files():
            ...
            try:
                ...
                warnings.append(
                    f"Failed to parse {file.relative_to(settings.PROJECT_DIR)} - any tags in that file will be ignored"
                )
    ...
    node_modules_dir = settings.PROJECT_DIR / "node_modules/"
