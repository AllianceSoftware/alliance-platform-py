...
Every command except ``--version``, ``--help``, ``help``, ``install``, and ``update-launcher``
requires an installed project with both ``config/dev.toml`` and ``pyproject.toml``. The project
root is, in order of precedence: the ``--project-dir`` option, the
...
.. list-table::
   ...
   * - :ref:`update-launcher <dev-command-update-launcher>`
     - ``[PATH] [--force] [--tool-source SOURCE]``
     - Regenerate ``bin/dev`` while preserving the project's existing configuration.
...
.. _dev-command-update-launcher:

``update-launcher``
~~~~~~~~~~~~~~~~~~~

.. program:: alliance-dev update-launcher

.. code-block:: bash

   uvx --upgrade --from alliance-platform-dev alliance-dev update-launcher [PATH] \
       [--tool-source SOURCE] [--force]

Regenerates only ``bin/dev`` from the running ``alliance-platform-dev`` release. Run it through
``uvx`` as shown when upgrading an existing project. The command recognizes launchers generated by
earlier releases, reports the previous and replacement tool sources, and leaves
``config/dev.toml``, Git hooks, Django settings, and ignore files unchanged. A missing launcher is
created.

.. option:: PATH

   The project root. It must contain ``pyproject.toml`` and cannot be combined with
   ``--project-dir``. Without either option, the nearest parent containing ``pyproject.toml`` is
   used.

.. option:: --tool-source SOURCE

   Override the ``uvx --from`` requirement written into ``bin/dev``. The default pins the version
   that supplied the command.

.. option:: --force

   Replace a ``bin/dev`` file that is not recognized as an Alliance Platform generated launcher.
   Review custom launcher behavior before using this option.
