...
from alliance_platform.storage.filesystem import FileSystemAsyncUploadStorage
...
from alliance_platform.storage.views import ViewProtocol
...
class FileSystemAsyncStorageUploadView(View):
    """View to handle a direct upload to the filesystem when using :class:`~alliance_platform.storage.filesystem.FileSystemAsyncUploadStorage`.
    ...
    Note that this view does works off of signed data generated by :class:`~alliance_platform.storage.filesystem.FileSystemAsyncUploadStorage.generate_upload_url`,
    ...
    The permission checking is done in :class:`~alliance_platform.storage.views.GenerateUploadUrlView`, which
    calls :class:`~alliance_platform.storage.filesystem.FileSystemAsyncUploadStorage.generate_upload_url`.

    The URL for this view is automatically registered when :class:`~alliance_platform.storage.filesystem.FileSystemAsyncUploadStorage`
...
class FileSystemAsyncStorageDownloadView(View):
    """View to handle a direct download from the filesystem when using :class:`~alliance_platform.storage.filesystem.FileSystemAsyncUploadStorage`.

    Each view is tied to a specific registry which you can specify in the ``registry`` kwarg (defaults to :data:`~alliance_platform.storage.registry.default_async_field_registry`).

    Note that this view does works off of signed data generated by :class:`~alliance_platform.storage.filesystem.FileSystemAsyncUploadStorage.generate_download_url`,
    and does not itself check any permissions. It does verify the signed data, including enforcing an expiry based on
