...
def resolve_vanilla_extract_cache_names(bundler: BaseBundler, filename: Path | str):
    ...
    if bundler.is_development():
        cache_fn = settings.FRONTEND_BUILD_CACHE / f"development-css-mappings/{cache_name}.json"
        import_script_filename = settings.FRONTEND_BUILD_CACHE / f"development-css-mappings/{cache_name}.ts"
        return cache_fn, import_script_filename
    return settings.FRONTEND_PRODUCTION_DIR / "production-css-mappings" / f"{cache_name}.json", None
...
class VanillaExtractClassMapping:
    ...
    def __init__(self, bundler: BaseBundler, filename: Path):
        ...
        self.filename = filename.relative_to(settings.PROJECT_DIR)
