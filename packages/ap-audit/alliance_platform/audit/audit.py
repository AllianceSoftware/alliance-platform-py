...
def create_audit_model_base(
    ...
    **kwargs,
    ...
    those events in ``manual_events``; to then trigger a manual event use :func:`~alliance_platform.audit.create_audit_event`.

    :code:`events` refers to the :external:py:class:`~pghistory.RowEvent` to be used which defaults to
    ...
    See :external:py:func:`pghistory.core.create_event_model` for the supported ``kwargs`` and more details
    ...
    Args:
        ...
        list_perm: The permission to use when showing audit events in list view. This should be a global
            permission (ie. doesn't accept a specific object). If not specified uses :func:`~alliance_platform.core.auth.resolve_perm_name`
        ...
        related_name (str): The primary way to identify the relation of the created model and the tracked model. If
            ...
            the provided fields (see :external:py:func:`pghistory.core.create_event_model`).
    """
...
def create_audit_event(object: models.Model, label: str) -> models.Model:
    ...
    the current user and URL in ``pgh_context`` for the created log event, and additional info can be
    added by wrapping :code:`create_audit_event` in :code:`with pghistory.context(**kwargs)`. See :external:py:class:`pghistory.context`
